...
class Renderer:
    ...
    def _render_grid(self, display, dx, dy, block_size=30):
        for row in range(self.game_grid.grid.shape[0]):
            for col in range(self.game_grid.grid.shape[1]):
                grid_rectangle = pygame.Rect(dx + col * block_size, \
                                             dy + row * block_size, block_size, block_size)
    ...
    def _render_grid_border(self, display, dx, dy, border_width=5):
        border_rectangle = pygame.Rect(dx - border_width, dy - border_width, \
                                       TETRIS_WIDTH + 30 + border_width, TETRIS_HEIGHT + 40)
